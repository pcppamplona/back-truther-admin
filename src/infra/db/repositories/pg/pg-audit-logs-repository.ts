...
export class PgAuditLogsRepository implements AuditLogsRepository {
  ...
  private async enrichAuditLogs(client: PoolClient, auditLogs: AuditLog[]): Promise<void> {
    if (auditLogs.length === 0) {
      return
    }

    const { userSenderIds, systemSenderIds, userTargetIds, systemTargetIds, externalUserIds } =
    ...
    auditLogs.forEach(log => {
      ...
      log.sender_name = senderIsUser
        ...
        : systemNames[log.sender_id] || this.getSystemFallbackName(log.sender_id)

      if (targetIsUser) {
        log.target_name = userNames[log.target_id] || this.getUserFallbackName(log.target_id)
        return
      }

      const baseTargetName = systemNames[log.target_id] || this.getSystemFallbackName(log.target_id)
      const externalUserName = log.target_external_id ? userNames[log.target_external_id] : undefined

      log.target_name = externalUserName ? `${baseTargetName} (${externalUserName})` : baseTargetName
    })
  }
  ...
  async findAll(): Promise<AuditLog[]> {
    ...
    try {
      const result = await client.query(`SELECT * FROM audit_logs ORDER BY created_at DESC`)
  ...
  async findById(id: number): Promise<AuditLog | null> {
    ...
    try {
      const result = await client.query(`SELECT * FROM audit_logs WHERE id = $1 LIMIT 1`, [id])
