...
export interface GetUsersTrutherWithWalletsUseCaseRequest {
  page: number
  limit: number
  address?: string
...
export class GetUsersTrutherWithWalletsUseCase {
  constructor(private usersTrutherRepository: UsersTrutherRepository) {}

  async execute({ page, limit, address, custodian }: GetUsersTrutherWithWalletsUseCaseRequest): Promise<PaginatedResult<UserTrutherWithWallet>> {
    const validPage = Math.max(1, page || 1)
    const validLimit = Math.max(1, Math.min(100, limit || 10))

    return this.usersTrutherRepository.findUsersTrutherWithWallets({
      page: validPage,
      limit: validLimit,
      address,
      custodian,
